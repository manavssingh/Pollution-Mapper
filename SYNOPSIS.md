...
* **Primary Technologies**: Python, Streamlit, Plotly, OpenStreetMap Nominatim, World Air Quality Index (WAQI) API, Open-Meteo Environmental API, GitHub Actions CI/CD
...
1. **Secondary Data**: Automated ingestion of real-time air quality metrics (AQI, PM2.5, PM10) from global monitoring networks via RESTful APIs.
2. **Primary Data**: Crowd-sourced and field-recorded acoustic decibel (dB) measurements collected using calibrated smartphone sound meters, augmented with diurnal WHO urban traffic baseline estimations for unmonitored global localities.
...
**Motivation**: To engineer a low-cost, zero-maintenance, open-access platform enabling researchers, civic planners, and citizens to visualize, map, and correlate localized air and noise pollution levels across any global city.

---

## 5. OBJECTIVES OF THE PROJECT
1. **Develop an Interactive GIS Web Application**: Build a responsive dashboard using Streamlit and Plotly to map environmental indicators over open-access map tiles.
2. **Implement Worldwide On-Demand Geocoding**: Allow instantaneous searching of any global city or neighborhood using OpenStreetMap Nominatim and real-time environmental APIs.
3. **Establish a Dual Primary & Secondary Data Ingestion Model**: Unify automated external air quality feeds with physical sound-meter primary observations and validated WHO acoustic baseline models.
4. **Engineer an Autonomous Cloud Data Pipeline**: Implement GitHub Actions cron workflows with encrypted secrets management to automatically fetch, parse, and commit longitudinal data daily.
5. **Conduct Multi-Pollutant Correlation Analysis**: Calculate the Pearson correlation coefficient between acoustic noise and particulate pollution to evaluate joint environmental exposure.
...
flowchart TD
    ...
    subgraph User Interface & Analytics
        ...
        Map["Interactive Scatter Mapbox/MapLibre"]
...
* **Network**: Broadband internet connection for real-time API queries and map rendering
...
## 8. EXPECTED OUTCOMES
1. A live, publicly accessible environmental dashboard providing locality-level resolution.
2. Capability to search and map any municipality globally with real-time AQI and contextual acoustic indices.
3. Automated longitudinal dataset expansion through cloud-scheduled data harvesting.
4. Statistical insight into the co-location of vehicular noise and atmospheric particulate concentrations.
5. Exportable open data formats (CSV) for downstream academic and municipal research.
